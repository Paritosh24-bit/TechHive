...
const industriesPath = path.join(__dirname, '..', 'src', 'services.js');
...
const arrayMatch = raw.match(/const SERVICES = (\[[\s\S]*\]);\s*\nexport default SERVICES;/);
if (!arrayMatch) {
  console.error('Could not locate SERVICES array literal in services.js');
  process.exit(1);
}
const SERVICES = Function(`"use strict"; return (${arrayMatch[1]});`)();
...
let changed = 0;
for (const industry of SERVICES) {
 ...
 *
 * This file is the single source of truth for every /services/:slug page.
 * ServicePage.jsx reads from this array purely by \`slug\` — it has no
 * knowledge of how many services exist or what they're called.
 *
...
console.log(`Pre-cleaned ${changed} services. Wrote ${(fs.statSync(industriesPath).size / 1024).toFixed(1)} KB to src/industries.js`);
